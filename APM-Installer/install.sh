...
mkdir apmInstaller #MaKe a new DIRectory called apminstaller, in which to store all temp files. 
cd apmInstaller #Change Directory to the apminstaller directory

arch=`uname -m` #Get architecture
if [ ${arch} == 'x86_64' ]; then #If 64 bit, then download the 64 bit version
 wget -O apmplanner.deb https://github.com/rspandolfi/DaVinciChallenge-Build-a-Drone-Workshop/releases/download/v2.0.1-64/apm_planner_2.0.15-7-gd94637c_ubuntu_trusty64.deb  #Download the APM Planner .deb from the daily build on 11/26/14. The newer version is needed because older versions had a hard dependency on libopenscenegraph80, which is not available on Ubuntu 14.04. The choice was to either compile libopenscenegraph80, or simply switch to a newer version of APM Planner. Since compiling libopenscenegraph80 would introduce unnecessary complexities, it was decided to use the newest version of APM Planner. 
else #If not 64 bit, then download the 32 bit version
 wget -O apmplanner.deb https://github.com/rspandolfi/DaVinciChallenge-Build-a-Drone-Workshop/releases/download/v2.0.1-32/apm_planner_2.0.15-7-gd94637c_ubuntu_trusty.deb
fi
